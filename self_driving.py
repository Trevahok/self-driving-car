...
def main(env, object_detection, start=(100, 100), goal=(100, 200), initial_direction=1):
    ...
    path_len = len(path)
    rand_numb = random.randint(0, path_len)
    break_point = start
    break_direction = initial_direction
    
    directions, orders_path = path_convert(path, start, initial_direction)
    ...
    for order in orders_path:
        ...
        if step == rand_numb:
            break_point = path[step]
            break_direction = directions[step]
            break
    ...
    new_env = mapping.rudimentary_map(1)
    new_env = mapping.env_enhancement(new_env)

    new_path = a_star_search(env, break_point, goal)
    mark_path(new_path, env)
    if new_path == False:
        return False

    new_directions, new_orders_path = path_convert(new_path, break_point, break_direction)

    step = 0
    for order in new_orders_path:
        if order > 0:
            if order == 3:
                turn_left()
            else:
